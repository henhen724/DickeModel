...
function dicke_hetrodyne_atom_only_meanfield_prob_feedback_smooth(; Nspin=10, κ=2π * 0.15, Δc=2π * 20, ωz=2π * 0.01, λ0=1.0, t_ramp=500.0, t_hold=0.0, λmod=0.0, ωmod=2π * 1e-6 * 500.0, P=5e-2, I=5e-4, D=0.0, PID_filt_T=500.0, PID_window_time=1000.0, set_point=0.0, Tfilt=100.0, tmax=500.0, taufilt=100, recordtimes=500, noise=nothing, save_noise=false)
    ...
    println("Alpha plus ", αplus, " Alpha minus ", αminus)

    spin_len = Nspin / 2.0

    #                Sx   Sy     Sz       Q    Jhetfilt(t) + Q(t) g'(t) g(t) 
    u0 = ComplexF64[0.0, 0.0, -spin_len, 0.0, 0.0, 0.0, λ0]
    ...
    function f!(du, u, p, t)
        ...
        du[6] = (p[1] - u[7]) / (2 * PID_filt_T)^2 - u[6] / PID_filt_T
        du[7] = u[6]
    ...
    num_noise = 2
    noise_prototype = zeros(ComplexF64, (7, num_noise))
    ...
    function PIDFeedback(integrator)
        ...
        if t > t_ramp && t_indx > 1
            ...
            error_sig[t_indx] = rescaled_autodensity!(Q) + 0.39 * abs(mean(Ifilt)) - set_point
            # else
            ...
            integrator.p[1] = max(0.5, integrator.p[1] - P * error_sig[t_indx] - I * sum(error_sig[1:t_indx]) * DeltaT - D * (error_sig[t_indx] - error_sig[t_indx-1]) / DeltaT)
